...
describe('Indicators', () => {
  ...
  it('gets Supertrend strategy', async () => {
    indicators.SuperTrend = await TradingView.getIndicator(
      'STD;Supertrend%Strategy',
    );
  ...
  it('gets MarketCipher B study', async () => {
    indicators.CipherB = await TradingView.getIndicator(
      'PUB;uA35GeckoTA2EfgI63SD2WCSmca4njxp',
    );
  ...
  it.concurrent(
    'gets performance data from SuperTrend strategy',
    async () => {
        ...
        SuperTrend.onUpdate(() => {
          ...
          console.log('Performances:', {
            total: {
              ...
              perf: `${
                Math.round((perfReport?.all?.netProfitPercent || 0) * 10000) /
                100
              } %`,
            },
            buy: {
              ...
              perf: `${
                Math.round((perfReport?.long?.netProfitPercent || 0) * 10000) /
                100
              } %`,
            },
            sell: {
              ...
              perf: `${
                Math.round((perfReport?.short?.netProfitPercent || 0) * 10000) /
                100
              } %`,
            },
          });
