...
module.exports = (client) =>
  class ChartSession {
    ...
    constructor() {
      this.#client.sessions[this.#chartSessionID] = {
        ...
        onData: (packet) => {
          ...
          if (
            typeof packet.data[1] === 'string' &&
            this.#studyListeners[packet.data[1]]
          ) {
     ...
     */
    fetchMore(number = 1) {
      this.#client.send('request_more_data', [
        this.#chartSessionID,
        '$prices',
        number,
      ]);
    ...
    replayStep(number = 1) {
      return new Promise((cb) => {
        ...
        this.#client.send('replay_step', [
          this.#replaySessionID,
          reqID,
          number,
        ]);
        this.#replayOKCB[reqID] = () => {
          cb();
        };
      });
    }
